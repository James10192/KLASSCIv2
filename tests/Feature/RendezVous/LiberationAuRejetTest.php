...
class LiberationAuRejetTest extends TestCase
{
    ...
    public function test_un_creneau_du_jour_pas_encore_fini_est_libere_aussi(): void
    {
        $r = $this->reservation($this->creneau('09:00', '09:30'));

        $liberees = app(LiberationRdv::class)->apresRejet($this->candidature($r));

        $this->assertCount(1, $liberees);
        $this->assertSame(StatutReservationRdv::Liberee, $r->fresh()->statut);
    }
    ...
    public function test_l_historique_et_les_faits_ne_sont_pas_reecrits(): void
    {
        ...
        $service = app(LiberationRdv::class);
        $service->apresRejet($this->candidature($passe));
        $service->apresRejet($this->candidature($recue));

        $this->assertSame(StatutReservationRdv::Confirmee, $passe->fresh()->statut, 'Un créneau terminé ne rend de place à personne.');
    ...
    public function test_on_ne_touche_qu_aux_reservations_du_dossier_rejete(): void
    {
        ...
        app(LiberationRdv::class)->apresRejet($this->candidature($rejetee));
