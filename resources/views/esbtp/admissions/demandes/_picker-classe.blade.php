{{-- Selecteur de classe des fenetres « inscrire » et « reinscrire ». Clone
     ...
     chaque classe, ce que le composant generique ne sait pas faire. Avec
     $bloquer, une classe complete (drapeau `complete` calcule par le serveur,
     la regle de l'enregistrement) reste visible mais ne se choisit pas. La
     reinscription ne verifie pas les places : elle ne bloque rien.
     Params : $modele (chemin Alpine de la valeur), $liste (expression Alpine), $bloquer (bool). --}}
    ...
    <div class="dmi-picker-menu" x-show="picker.ouvert" x-cloak role="listbox">
        ...
        <div class="dmi-picker-liste">
            <template x-for="c in filtrerClasses({{ $liste }})" :key="c.id">
                <button type="button" class="dmi-picker-option" role="option" :aria-selected="{{ $modele }} === c.id" :disabled="{{ $bloquer ? 'c.complete' : 'false' }}"
