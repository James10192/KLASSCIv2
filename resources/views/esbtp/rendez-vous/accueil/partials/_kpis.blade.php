...
<div class="rdv-kpis">
    ...
    <div class="rdv-kpi">
        <div class="rdv-kpi-icon"><i class="fas fa-user-xmark"></i></div>
        <div><div class="rdv-kpi-value">{{ $_c['absents'] }}</div><div class="rdv-kpi-label">Absentes à reprogrammer{{ $_c['reprogrammees'] > 0 ? ', '.$_c['reprogrammees'].' déjà reprogrammée'.($_c['reprogrammees'] > 1 ? 's' : '') : '' }}</div></div>
    </div>
</div>
<div class="rac-avancement" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="{{ $_pct }}" aria-label="Familles traitées">
    <div class="rac-avancement-barre"><span style="width: {{ $_pct }}%"></span></div>
    <span>{{ $_traites }} / {{ $_c['attendus'] }} traitées</span>
</div>
