...
<div class="mm-page">
    ...
    <div class="mm-list" id="mmList">
        @forelse($messages as $message)
            @include('esbtp.annonces._message-etudiant')
        @empty
            <div class="mm-empty">
    ...
    <x-liste-infinie :paginateur="$messages" cible="#mmList" libelle="messages" />
</div>
@endsection
...
(function () {
    ...
    // Les compteurs viennent du serveur et portent sur tous les messages : les
    // cartes affichees n'en sont qu'une partie, la suite se chargeant au
    // defilement. Lire un message ne fait que decompter.
    const compteurs = @json($stats);
    let filtreCourant = 'all';
        ...
        .then(data => {
            if (data && data.success) {
                ...
                if (card && card.dataset.mmState === 'unread') {
                    compteurs.unread = Math.max(0, compteurs.unread - 1);
                }
                if (card) {
    ...
    // Les messages charges au defilement suivent l'onglet choisi.
    document.addEventListener('liste-infinie:ajout', function () {
        applyFilter(filtreCourant);
    });
