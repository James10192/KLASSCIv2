                ...
                <table class="cd-table">
                    ...
                    <tbody id="cd-tbody">
                        @foreach($candidatures as $c)
                            @include('esbtp.inscriptions.candidatures._ligne', ['c' => $c])
                        @endforeach
                    </tbody>
                </table>
            </div>
            <x-liste-infinie :paginateur="$candidatures" cible="#cd-tbody" libelle="candidatures" />
        @endif
    </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (dossierEl) {
        ...
        // Delegation : les lignes arrivees au defilement ouvrent leur dossier
        // comme celles du premier affichage.
        document.addEventListener('click', function (ev) {
            var voir = ev.target.closest('.cd-voir');
            var ligne = ev.target.closest('tr.cd-ligne');
            if (!ligne) return;
            if (!voir && ev.target.closest('a, button, form, input, select, textarea')) {
    ...
    document.addEventListener('click', function (ev) {
        var bouton = ev.target.closest('[data-rejet-id]');
        if (!bouton) return;
        (function () {
