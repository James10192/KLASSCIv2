...
@section('title', 'Fiche Caissier — ' . $caissier->name . ' — KLASSCI')

@section('styles')
<link rel="stylesheet" href="{{ asset('css/dashboard-moderne.css') }}">
<style>
/* ===================================================================
   CAISSIER SHOW — Premium Design — KLASSCI Design System
   Namespace: cs- (caissier-show)
=================================================================== */
...
/* Avatar */
.cs-hero-avatar { position: relative; flex-shrink: 0; }
...
.cs-hero-avatar-circle img { width: 100%; height: 100%; object-fit: cover; display: block; }
.cs-hero-status {
...
.cs-quick-card {
    ...
    padding: 20px 12px; border-radius: var(--cs-radius);
    border: 1px solid var(--cs-border);
    background: var(--cs-card);
    text-decoration: none; color: var(--cs-text);
    transition: all .18s;
}
.cs-quick-card:hover {
    border-color: var(--cs-blue); color: var(--cs-blue);
...
.cs-quick-card i { font-size: 1.6rem; }
.cs-quick-card span { font-size: .82rem; font-weight: 600; text-align: center; }
...
/* -- Empty state --------------------------------------------------- */
.cs-empty { text-align: center; padding: 40px 20px; color: var(--cs-muted); }
.cs-empty-icon {
...
<div class="cs-page">
    ...
    <div class="cs-hero">
        <div class="cs-hero-inner">
            ...
            <div class="cs-hero-avatar">
                <div class="cs-hero-avatar-circle">
                    @if($caissier->profile_photo_path ?? false)
                        <img src="{{ asset('storage/' . $caissier->profile_photo_path) }}" alt="{{ $caissier->name }}">
                    @else
                        {{ strtoupper(substr($caissier->name, 0, 2)) }}
                    @endif
                </div>
                <div class="cs-hero-status {{ $caissier->is_active ? 'active' : 'inactive' }}"></div>
            </div>

            {{-- Info --}}
            <div class="cs-hero-text">
                <h1 class="cs-hero-name">{{ $caissier->name }}</h1>
                <p class="cs-hero-sub">Service Caisse — Pré-inscriptions &amp; encaissements</p>
                <div class="cs-hero-pills">
                    <span class="cs-hero-pill">
                        <i class="fas fa-cash-register"></i> Caissier
                    </span>
                    <span class="cs-hero-pill {{ $caissier->is_active ? 'green' : '' }}">
                        <i class="fas fa-circle" style="font-size:.5rem"></i>
                        {{ $caissier->is_active ? 'Actif' : 'Inactif' }}
                    </span>
                    <span class="cs-hero-pill muted">
                        <i class="fas fa-calendar-alt"></i>
                        Depuis {{ $caissier->created_at->format('M Y') }}
                    </span>
                    @if($caissier->last_login_at)
                    <span class="cs-hero-pill muted">
                        <i class="fas fa-clock"></i>
                        {{ $caissier->last_login_at->diffForHumans() }}
                    </span>
                    @endif
                </div>
            </div>
            ...
            <div class="cs-hero-actions">
                <div class="cs-hero-btns">
                    <a href="{{ route('esbtp.caissiers.edit', $caissier) }}" class="cs-hero-btn primary">
        ...
        <div class="cs-kpi-strip">
            <div class="cs-kpi">
                <i class="fas fa-cash-register cs-kpi-icon"></i>
                <div>
                    <div class="cs-kpi-val">Caissier</div>
                    <div class="cs-kpi-lbl">Role</div>
                </div>
            </div>
            <div class="cs-kpi">
                <i class="fas fa-circle cs-kpi-icon" style="font-size:.6rem; color: {{ $caissier->is_active ? '#10b981' : '#94a3b8' }};"></i>
                <div>
                    <div class="cs-kpi-val">{{ $caissier->is_active ? 'Actif' : 'Inactif' }}</div>
                    <div class="cs-kpi-lbl">Statut</div>
                </div>
            </div>
            <div class="cs-kpi">
                <i class="fas fa-calendar-check cs-kpi-icon"></i>
                <div>
                    <div class="cs-kpi-val">{{ $caissier->created_at->diffInDays(now()) }}</div>
            ...
            <div class="cs-kpi">
                ...
                <div>
                    <div class="cs-kpi-val">{{ $caissier->last_login_at ? $caissier->last_login_at->format('d/m/Y') : '---' }}</div>
    ...
    <div class="cs-content">
        ...
        {{-- Flash messages --}}
        @if(session('success'))
        <div class="alert alert-success alert-dismissible fade show rounded-3 mb-4" style="border: none; box-shadow: var(--cs-shadow);">
            <i class="fas fa-check-circle me-2"></i>{{ session('success') }}
            <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
        </div>
        @endif
        @if(session('error'))
        <div class="alert alert-danger alert-dismissible fade show rounded-3 mb-4" style="border: none; box-shadow: var(--cs-shadow);">
            ...
            <div class="cs-grid-2">
                ...
                <div class="cs-card">
                    ...
                    <div class="cs-info-row">
                        ...
                        <span class="cs-info-value">{{ $caissier->name }}</span>
                    </div>
                    <div class="cs-info-row">
                        <span class="cs-info-label"><i class="fas fa-envelope"></i> Email</span>
                        <span class="cs-info-value">{{ $caissier->email ?: 'Non renseigne' }}</span>
                    </div>
                    <div class="cs-info-row">
                        <span class="cs-info-label"><i class="fas fa-phone"></i> Telephone</span>
                        <span class="cs-info-value">{{ $caissier->phone ?? $caissier->telephone ?? 'Non renseigne' }}</span>
                    </div>
                    <div class="cs-info-row">
                        <span class="cs-info-label"><i class="fas fa-at"></i> Nom d'utilisateur</span>
                        <span class="cs-info-value">{{ $caissier->username }}</span>
                    </div>
                </div>
                ...
                <div class="cs-card">
                    ...
                    <div class="cs-info-row">
                        <span class="cs-info-label"><i class="fas fa-user-tag"></i> Role</span>
                        <span class="cs-info-value">Caissier</span>
                    </div>
                    <div class="cs-info-row">
                        <span class="cs-info-label"><i class="fas fa-toggle-on"></i> Statut du compte</span>
                        <span class="cs-info-value">
                            <span class="cs-badge {{ $caissier->is_active ? 'success' : 'danger' }}">
                    ...
                    <div class="cs-info-row">
                        ...
                        <span class="cs-info-value">{{ $caissier->first_login_at ? $caissier->first_login_at->format('d/m/Y H:i') : 'Jamais connecte' }}</span>
                    </div>
                    <div class="cs-info-row">
                        <span class="cs-info-label"><i class="fas fa-clock"></i> Derniere connexion</span>
                        <span class="cs-info-value">{{ $caissier->last_login_at ? $caissier->last_login_at->format('d/m/Y H:i') : 'Jamais connecte' }}</span>
                    </div>

                    {{-- Contact rapide --}}
                    @if($caissier->email || $caissier->phone)
                    <div style="margin-top: 20px; padding-top: 16px; border-top: 1px solid var(--cs-border);">
                        ...
                        <div class="cs-contact-item">
                            ...
                            <a href="mailto:{{ $caissier->email }}">{{ $caissier->email }}</a>
                        </div>
                        @endif
                        @if($caissier->phone ?? $caissier->telephone ?? false)
                        <div class="cs-contact-item">
                            <div class="cs-contact-icon"><i class="fas fa-phone"></i></div>
                            <a href="tel:{{ $caissier->phone ?? $caissier->telephone }}">{{ $caissier->phone ?? $caissier->telephone }}</a>
                        </div>
                        @endif
                    </div>
                    @endif
                ...
                <div class="cs-grid-2">
                    <div>
                        <div class="cs-info-row">
                            ...
                            <span class="cs-info-value">#{{ $caissier->id }}</span>
                        </div>
                        <div class="cs-info-row">
                            <span class="cs-info-label"><i class="fas fa-calendar-plus"></i> Cree le</span>
                            <span class="cs-info-value">{{ $caissier->created_at->format('d/m/Y H:i') }}</span>
                        </div>
                    </div>
                    <div>
                        <div class="cs-info-row">
                            <span class="cs-info-label"><i class="fas fa-edit"></i> Modifie le</span>
                            <span class="cs-info-value">{{ $caissier->updated_at->format('d/m/Y H:i') }}</span>
                        </div>
                        @if($caissier->created_by)
                        <div class="cs-info-row">
                            <span class="cs-info-label"><i class="fas fa-user-shield"></i> Cree par</span>
                            <span class="cs-info-value">{{ $caissier->createdBy->name ?? 'N/A' }}</span>
                        </div>
                        @endif
                    </div>
                </div>
        ...
        <div class="cs-panel" id="cs-tab-account">
            <div class="cs-grid-2">
                <div class="cs-card">
                    ...
                    @if(session('new_password'))
                        <div class="cs-password-alert">
                            ...
                            <p class="cs-password-hint"><i class="fas fa-info-circle"></i> Communiquez ces identifiants au caissier.</p>
                        </div>
                    @endif
                ...
                <div class="cs-card">
                    ...
                    <div class="cs-info-row">
                        <span class="cs-info-label"><i class="fas fa-sign-in-alt"></i> Premiere connexion</span>
                        <span class="cs-info-value">{{ $caissier->first_login_at ? $caissier->first_login_at->format('d/m/Y H:i') : 'Jamais' }}</span>
                    </div>
                    <div class="cs-info-row">
                        <span class="cs-info-label"><i class="fas fa-clock"></i> Derniere connexion</span>
                        <span class="cs-info-value">{{ $caissier->last_login_at ? $caissier->last_login_at->format('d/m/Y H:i') : 'Jamais' }}</span>
                    </div>
                    <div class="cs-info-row">
                        <span class="cs-info-label"><i class="fas fa-calendar"></i> Compte cree le</span>
                        <span class="cs-info-value">{{ $caissier->created_at->format('d/m/Y') }}</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="cs-grid-2">
                <div class="cs-card">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 10px;">
                        <a href="{{ route('esbtp.caissiers.edit', $caissier) }}" class="cs-action-btn">
                            <i class="fas fa-edit"></i>
                            <span>Modifier le profil</span>
                        </a>

                        @if($caissier->is_active)
                        <form action="{{ route('esbtp.caissiers.toggle-status', $caissier) }}" method="POST"
                              onsubmit="return confirm('Desactiver ce caissier ?')">
                            @csrf
                            @method('PATCH')
                        ...
                        @else
                        <form action="{{ route('esbtp.caissiers.toggle-status', $caissier) }}" method="POST"
                              onsubmit="return confirm('Activer ce caissier ?')">
                            @csrf
                            @method('PATCH')
                ...
                <div class="cs-card">
                    ...
                    <p style="font-size: .85rem; color: var(--cs-muted); margin: 0 0 16px;">
                        Cette action desactivera le caissier et retirera son role. Elle est irreversible.
                    </p>

                    <form action="{{ route('esbtp.caissiers.destroy', $caissier) }}" method="POST"
                          onsubmit="return confirm('Supprimer definitivement ce caissier ? Cette action est irreversible.')">
                        @csrf
                        @method('DELETE')
...
<div class="modal fade" id="resetPasswordModal" tabindex="-1" aria-labelledby="resetPasswordModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 16px; border: none; box-shadow: 0 10px 40px rgba(0,0,0,0.2); overflow: hidden;">
            ...
            <form id="resetPasswordForm" method="POST" action="{{ route('esbtp.caissiers.reset-password', ['caissier' => $caissier->id]) }}">
                @csrf
                <div class="modal-body" style="padding: 2rem;">
                        ...
                        <div class="d-flex align-items-start gap-3">
                            ...
                            <div style="flex-grow: 1;">
                                ...
                                <div style="color: var(--cs-muted); font-size: 0.9rem;">
                                    Cette action va reinitialiser le mot de passe a <strong>"{{ mot_de_passe_par_defaut() }}"</strong> pour le caissier
                                    <strong>{{ $caissier->name }}</strong>. Le caissier devra changer son mot de passe a la premiere connexion.
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div style="
                            ...
                            font-weight: 500;
                        ">
                            {{ $caissier->name }} ({{ $caissier->email ?: $caissier->username }})
                        </div>
                    </div>
                    ...
                    <div id="newPasswordDisplay" style="display: none;" class="mb-3">
                        ...
                        <div class="form-text text-center mt-2" style="color: #047857;">
                            ...
                            Communiquez ce mot de passe au caissier. Il devra le changer a la premiere connexion.
                        </div>
                    </div>
                </div>
...
document.getElementById('resetPasswordModal').addEventListener('hidden.bs.modal', function() {
    ...
    document.getElementById('resetPasswordBtn').innerHTML = '<i class="fas fa-key me-1"></i>Reinitialiser a ' + {{ Js::from(mot_de_passe_par_defaut()) }};
