{{--
    Moteur de rendu des graphiques premium : palette, tooltips, clic, et
    l'API imperative window.klassciGraphique pour les ecrans dont les donnees
    arrivent en AJAX.

    Charge par <x-role-chart>, et directement par les pages qui pilotent
    leurs graphiques depuis Alpine sans gabarit statique.
--}}
@once
@push('styles')
<style>
...
(function () {
    ...
    // Version imperative, pour les ecrans dont les donnees arrivent en AJAX et
    // changent au fil des filtres. Meme palette, memes tooltips, meme clic :
    // un second moteur de rendu divergerait a la premiere retouche.
    window.klassciGraphique = function (canvas, type, data, surcharges) {
        if (!canvas || typeof Chart === 'undefined') return null;
        var payload = { data: data || {}, options: surcharges || {} };
        ...
        canvas.dataset.chartPayload = JSON.stringify(payload);
        canvas.dataset.chartRendu = '';
        rendre(canvas);
