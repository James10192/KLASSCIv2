...
- **21/10** : Refonte complète du formulaire `seances-cours.edit` (UI identique à create + script unique) → init type conserve la couleur existante, fallback enseignants quand planification vide, grille de disponibilité toujours visible, validation côté client harmonisée. Les autres cartes de type sont verrouillées avec un message guidant la suppression/recréation. Ajustement `ESBTPSeanceCoursController::addExistingSessionsToAvailability()` pour ignorer la séance en cours d'édition (plus d'alerte "enseignant occupé" quand on modifie la même plage) et nouveau comportement pour les séances `homework` (aucun enseignant requis ni grille de dispo, formulaire épuré façon `evaluations.create`). Les devoirs n'impactent plus le volume horaire des planifications (filtres `type=course` + listener ignoré) et l'évaluation générée est désormais reliée/synchronisée (stockage `homework_evaluation_id`, update & delete automatiques, enseignant nul). Migration artisan `add_homework_evaluation_id_to_esbtp_seance_cours_table` + helper `combineDateAndTime()` pour fiabiliser les horodatages devoirs, suppression du type ambigu dans les requêtes (qualifier `esbtp_seance_cours.type`).
- **21/10** : Harmonisation prévisualisation / export PDF des emplois du temps (pattern "liste d'appel") → header moderne avec actions, KPI volume/enseignants/matières, carte établissement avec branding `SettingsHelper`, distribution par type de séance, top matières. Grille hebdomadaire factorisée (`partials/timetable-grid`) utilisée côté web & PDF (rowspan multi-crénaux, teintes cohérentes). Service `ESBTPPDFService::genererEmploiTempsPDF()` enrichi (config école, logo base64, métriques, nouvelles couleurs). Nouveau template `pdf/emploi-temps.blade.php` en paysage, légende, pourcentage par type; `preview-pdf` bascule sur layout dashboard redesign. Ajustement palette → gradients KLASSCI `#0453cb → #5e91de` et export PDF réduit à l'entête institutionnelle + grille hebdo.
- **26/10** : Correctif placement grille hebdomadaire (minutes ≠ 00) sur `show`, `preview` & PDF → recalcul index + découpe dynamique rowSpan pour éviter le glissement des séances sur le jour suivant. Synchronisation couleurs/types (affichage du type, usage couleur séance, légende PDF). Header PDF réaligné sur la preview (gradient KLASSCI, cartes horizontales). Planification académique : surface désormais tous les enseignants embarqués dans la planification (principal + secondaires) et badges multi-professeurs pour guider le choix lors de la création de séance.
