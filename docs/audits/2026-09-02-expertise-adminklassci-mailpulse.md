...
### Intégration groupe : KLASSCI, LMS, applications externes (15 constats, 2 critiques)
...
- **Câblage MailPulse d'une école** (mailpulse #21, KLASSCI #821) : une organisation MailPulse par établissement, `Contact.externalId` unique, templates WhatsApp au lieu du texte libre (sinon chaque notification hors fenêtre de 24 h coûte un SMS), les dix valeurs de configuration sortent du `.env` pour entrer dans les réglages et une commande `mailpulse:setup`.

Pour le LMS, la conclusion est plus simple : il n'a aujourd'hui aucune intégration MailPulse (front Angular qui ne parle qu'à KLASSCI). La promesse « moteur de notifications du groupe » n'est vraie que pour KLASSCI, et le pattern d'intégration d'un nouveau produit (une application externe par produit et par établissement, SDK minimal en TypeScript et PHP) reste à écrire (mailpulse #20).
