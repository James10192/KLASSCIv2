...
class OfficialDocumentService
{
    ...
    private function persistIssuedPv(ESBTPLMDJury $jury, ?OfficialDocument $previous, array $identity, array $snapshot, string $contents, string $path, User $actor): OfficialDocument
    {
        ...
        $document = OfficialDocument::query()->create(['document_type' => OfficialDocument::TYPE_LMD_JURY_PV, 'source_type' => ESBTPLMDJury::class, 'source_id' => $jury->id, 'series_key' => $seriesKey, 'version' => $identity['version'], 'reference' => $identity['reference'], 'status' => OfficialDocument::STATUS_VALID, 'valid_series_key' => $seriesKey, 'disk' => OfficialDocumentStorage::DISK, 'path' => $path, 'original_name' => $identity['number'].'-v'.$identity['version'].'.pdf', 'mime_type' => 'application/pdf', 'size_bytes' => strlen($contents), 'checksum_sha256' => hash('sha256', $contents), 'snapshot' => $snapshot, 'snapshot_sha256' => hash('sha256', $this->snapshots->canonicalJson($snapshot)), 'rules_version' => 'lmd-academic-profile-v1', 'template_version' => 'lmd-jury-pv-v3', 'renderer_version' => 'dompdf-v2', 'verification_code_digest' => hash('sha256', $identity['verification_code']), 'issued_by' => $actor->id, 'issued_at' => $snapshot['issuance']['issued_at'], 'supersedes_document_id' => $previous?->id]);
