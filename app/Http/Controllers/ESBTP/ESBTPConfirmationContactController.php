...
class ESBTPConfirmationContactController extends Controller
{
    ...
    public function candidature(ConfirmerContactRequest $request, ESBTPCandidature $candidature): RedirectResponse|JsonResponse
    {
        return $this->confirmer($request, $candidature);
    }

    public function reinscription(ConfirmerContactRequest $request, ESBTPReinscriptionDemande $demande): RedirectResponse|JsonResponse
    {
        return $this->confirmer($request, $demande);
    }

    private function confirmer(ConfirmerContactRequest $request, Model $demande): RedirectResponse|JsonResponse
    {
        [$resultat, $replanifiees] = $this->confirmation->confirmer($demande, $request->empreinte(), (int) $request->user()->id);

        return match ($resultat) {
            ConfirmationContactEcole::MODIFIE_ENTRE_TEMPS => $this->repondre($request, false, 'Ce dossier a changé depuis l\'affichage de la page (nouveau dépôt ou autre agent). Rechargez-la et vérifiez le contact avant de confirmer.'),
            ConfirmationContactEcole::PAS_A_CONFIRMER => $request->expectsJson()
                ? $this->repondre($request, true, 'Le contact de ce dossier n\'est pas à confirmer.')
                : back()->with('info', 'Le contact de ce dossier n\'est pas à confirmer.'),
            default => $this->repondre($request, true, 'Contact confirmé : le dossier peut être convoqué.'.($replanifiees > 0
