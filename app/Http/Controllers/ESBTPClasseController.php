...
class ESBTPClasseController extends Controller
{
    ...
    public function show(Request $request, ESBTPClasse $classe)
    {
        ...
        if ($classe->systeme_academique === 'LMD' && $lmdMatieres->isNotEmpty()) {
            $lmdUesAvecEcues = $this->buildLmdUesAvecEcues($lmdMatieres, $lmdVolumeBudget);
        }
    ...
    /**
     * Groupe les ECUEs d'une classe LMD par UE avec agregats CM/TD/TP par UE.
     * Bucket "Hors UE" pour les ECUEs sans unite_enseignement_id.
     * Source canonique : $lmdMatieres (deja charge via ESBTPPlanificationAcademique).
     * Heures realisees viennent de $lmdVolumeBudget (keye par matiere_id).
     */
    private function buildLmdUesAvecEcues($lmdMatieres, array $lmdVolumeBudget): \Illuminate\Support\Collection
    {
        return $lmdMatieres
