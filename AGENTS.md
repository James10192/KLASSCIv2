...
>
> **OpenCode** lit ce fichier. Les rules, hooks et skills sont ceux de Claude : `.claude/` (junctions `.opencode/rules|hooks|skills`). Recherche et briefs d’instance : `.opencode/research/`, `.opencode/structuration/`. Après clone : `powershell -File scripts/link-opencode-claude.ps1`.
...
Chaque instance a sa propre branche Git du même nom (snapshot de `presentation` synchronisé périodiquement). Voir [.Codex/rules/tenant-branches.md](.Codex/rules/tenant-branches.md).
...
**Source de vérité** : `config/permissions.php` (registry centralisé). Lecture via `App\Services\PermissionRegistry`. Voir [.Codex/rules/permissions.md](.Codex/rules/permissions.md).
